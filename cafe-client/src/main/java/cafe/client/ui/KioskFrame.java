...
/**
 * Full-screen kiosk when LOCKED; collapses to a 15%-wide right-side sidebar
 * when UNLOCKED so the user can access the desktop.
 *
 * Merged from both branches:
 *  - Sidebar layout (zaagii): dark theme, Mongolian labels, ad panel, progress bar, clock
 *  - Colour urgency warnings (U1): amber at 5 min, red + flash at 1 min
 *  - Audio beep (U2): beep at 60 s remaining and on expiry
 *  - Station # as headline on locked card (U5)
 *  - Corrected pricing: 5,000₮ = 60 мин
 */
public final class KioskFrame extends JFrame implements ServerConnection.Listener {
    ...
    private static final int WARN_AMBER_SECS = 300;  // 5 min → amber
    private static final int WARN_RED_SECS   = 60;   // 1 min → red + flash

    private static final Color COLOR_GREEN  = new Color(46, 204, 113);
    private static final Color COLOR_AMBER  = new Color(241, 196, 15);
    ...
    // Progress bar (top of info panel)
    private final JProgressBar sideBar = new JProgressBar(0, 100);

    // Clocks — locked card and sidebar share separate labels
    ...
    /** Locked card — station # is the big headline (U5), subtitle below. */
    private JPanel buildLockedCard() {
        ...
        stationLbl.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 72));

        JLabel title = new JLabel("Та төлбөрөө төлнө үү", SwingConstants.CENTER);
        title.setForeground(new Color(160, 175, 210));
        ...
        divider.setPreferredSize(new Dimension(500, 2));
        divider.setBackground(new Color(60, 70, 100));
        divider.setOpaque(true);

        clockLabel.setForeground(new Color(150, 165, 200));
        clockLabel.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 28));

        GridBagConstraints c = new GridBagConstraints();
        c.gridx = 0; c.anchor = GridBagConstraints.CENTER;
        c.gridy = 0; c.fill = GridBagConstraints.NONE;       c.insets = new Insets(0, 20, 20, 20); p.add(stationLbl, c);
        c.gridy = 1;                                          c.insets = new Insets(0, 20, 16, 20); p.add(title,      c);
        c.gridy = 2;                                          c.insets = new Insets(0, 20, 24, 20); p.add(badge,      c);
        c.gridy = 3; c.fill = GridBagConstraints.HORIZONTAL; c.insets = new Insets(0, 20, 20, 20); p.add(divider,    c);
        c.gridy = 4; c.fill = GridBagConstraints.NONE;       c.insets = new Insets(0, 20, 8,  20); p.add(clockLabel, c);
        return p;
    }

    /** Sidebar card — shown when UNLOCKED; 15% width, right-anchored. */
    ...
    private JPanel buildStationHeader() {
        ...
        p.add(label("STATION",             10, Font.BOLD,  COLOR_DIM));
        p.add(label("#" + cfg.stationId(), 18, Font.BOLD,  COLOR_LIGHT));
        return p;
    }
    ...
    private JPanel buildAdPanel() {
        ...
        JLabel price      = label("5,000₮ = 60 мин",    15, Font.BOLD,  COLOR_GREEN);
        JLabel priceNote  = label("(83₮/мин)",           11, Font.PLAIN, COLOR_DIM);

        int row = 0;
        c.gridy = row++; c.insets = new Insets(16, 10, 2,  10); p.add(brand,       c);
        c.gridy = row++; c.insets = new Insets(2,  10, 12, 10); p.add(slogan,      c);
        c.gridy = row++; c.insets = new Insets(0,  6,  8,  6);  p.add(separator(), c);
    ...
    private void switchToSidebar() {
        ...
        int sideW = Math.max(130, sb.width * 15 / 100);
    ...
    private void tick() {
        if (secondsRemaining <= 0) {
            secondTimer.stop();
            switchToFullScreen();
            Toolkit.getDefaultToolkit().beep();   // U2: beep on expiry
    ...
    /** U1: urgency colours — green → amber at 5 min → red+flash at 1 min. */
    private void renderTime() {
        ...
        } else if (secondsRemaining <= WARN_AMBER_SECS) {
            color = COLOR_AMBER;
        } else {
            color = COLOR_GREEN;
        }
