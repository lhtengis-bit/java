...
public final class ClientRegistry {
    ...
    public void register(int stationId, ClientHandler handler) {
        ...
        if (previous != null && previous != handler) {
            ...
            } catch (Exception ignored) { /* already swallowed inside closeQuietly */ }
        }
    }
    ...
    /** Send {@code command} to every currently-connected kiosk. */
    public void broadcast(String command) {
        handlers.values().forEach(h -> h.send(command));
    }
