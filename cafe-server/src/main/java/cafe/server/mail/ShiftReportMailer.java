...
public final class ShiftReportMailer {
    ...
    private String buildBody(List<CashTransaction> tx, BigDecimal total) {
        ...
        sb.append("PC CAFE — END OF SHIFT REPORT\n")
          .append("Date: ").append(LocalDate.now()).append("\n")
          .append("Transactions: ").append(tx.size()).append("\n\n");

        // Per-station breakdown
        sb.append("PER-STATION SUMMARY\n");
        sb.append("-------------------\n");
        ...
        if (byStation.isEmpty()) {
            ...
            for (Map.Entry<Integer, List<CashTransaction>> entry : byStation.entrySet()) {
                ...
                sb.append(String.format("  Station %3d : %2d tx   ₮%s%n",
                    entry.getKey(), entry.getValue().size(), subtotal.toPlainString()));
            }
        }
        sb.append("\n");

        // Full transaction listing
        sb.append("FULL TRANSACTION LOG\n");
        sb.append(String.format("%-6s %-8s %-14s %-20s%n", "TXN", "STN", "AMOUNT (₮)", "TIME"));
        sb.append("----------------------------------------------------\n");
        DateTimeFormatter tf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        for (CashTransaction t : tx) {
            sb.append(String.format("%-6d %-8d %-14s %-20s%n",
                t.getTransactionId(),
                t.getStationId(),
        ...
        sb.append("----------------------------------------------------\n");
        sb.append(String.format("TOTAL COLLECTED : ₮%s%n", total.toPlainString()));
        return sb.toString();
    }
}
